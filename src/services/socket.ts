...
class SocketService {
  ...
  private eventEmitters: Record<SOCKET_SERVER_EVENTS, Emitter> = {
    ...
    [SOCKET_SERVER_EVENTS.USER_LEAVE]: (callback: () => void) =>
      this.socket.emit(
  ...
  private _onUserJoined(player: IPlayer) {
    ...
    const updatedPlayers = [...this.socketUsers.get(), matchingPlayer];

    this.socketUsers.set(updatedPlayers);
  }

  private _onUserLeft(summonerId: string) {
    const newUsers = [...this.socketUsers.get()];
    const disconnectedUser = pullOne(
      newUsers,
  ...
  private _joinRoomCallback(players: IPlayer[]) {
